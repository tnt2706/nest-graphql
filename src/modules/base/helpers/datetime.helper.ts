...
export class DateTimeHelper {
  ...
  static getFirstDateOfCurrentMonth(): number {
    return Number(moment().startOf('month').format('x'));
  }

  static getLastDateOfCurrentMonth(): number {
    return Number(moment().endOf('month').format('x'));
  }
  ...
  static convertToDisplay(
    ...
    return this.isValidTimezone(timezone)
      ? moment(`${timestamp}`, 'x').tz(timezone).format(timeFormat)
  ...
  static getFirstDayOfPreviousMonth(): number {
    return Number(moment().subtract(1, 'months').startOf('month').format('x'));
  }

  static getLastDayOfPreviousMonth(): number {
    return Number(moment().subtract(1, 'months').endOf('month').format('x'));
  }
